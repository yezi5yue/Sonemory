...
- 完整 JSON 本地备份覆盖资料库、设置、暂停会话、最近记录和逐项进度；恢复前验证 schema 和实体引用。
- 恢复支持整库替换，或仅合并新 ID 且保留本机冲突；写入失败自动回滚。
- 资料卡可查看逐项累计明细，分别显示答对、答错、未听清、提示使用、最近结果和建议。
- 提示使用从知识答错中拆分为 `assisted`；旧进度缺失字段继续兼容。
- 连续两次识别失败后纯语音引导麦克风设备/权限自检和重试；仍失败时自动切换只听跟读，并随暂停会话恢复。

最近一次验证结果：43 个 Node 测试全部通过；静态检查覆盖 39 个必需文件；浏览器实测通过备份生成、JSON 预检、保留本机的合并恢复、逐项明细弹窗和原资料保持，控制台无警告或错误。真实麦克风音频未被采集，语音恢复由模拟设备测试和状态机测试覆盖。
...
| `src/backup.js` | 备份格式、完整性校验、冲突预检和本机优先合并 |
...
逐项 mastery 兼容旧记录，并累计 `correct`、`incorrect`、`recognitionFailures`、`assisted`、`attempts`、`lastOutcome`、`lastPracticedAt`。完整备份格式为 `sonemory-local-backup`，备份 `schemaVersion` 当前为 1；它不包含只存在页面内存的 AI 临时令牌或原始录音。
...
访问 `http://127.0.0.1:4173/?v=0.6.0`。修改静态资源后要更新版本查询参数和 Service Worker 缓存名，否则可能看到旧缓存。
...
- `v0.6.0`：完整备份恢复、逐项原因明细、纯语音识别恢复和只听跟读降级。
